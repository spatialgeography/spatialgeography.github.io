---
title: 12-quick-formulas-reference
id: 20260430146197
---
## Quick Formulas Reference

### Complete Formula Sheet (NET Notes — Pulakesh Pradhan)

| Measure | Formula |
|:---|:---|
| Arithmetic Mean | x̄ = Σfx / Σf |
| Mode | L + h(f1−f0) / [(f1−f0)−(f2−f1)] |
| Mode (approx) | 3 Median − 2 Mean |
| Range | Xmax − Xmin |
| Coefficient of Range | (Xmax−Xmin) / (Xmax+Xmin) |
| Quartile Deviation | (Q3−Q1) / 2 |
| Coefficient of QD | (Q3−Q1) / (Q3+Q1) |
| Mean Deviation | (1/N) Σ\|x−A\| |
| Standard Deviation | σ = √[(1/N) Σ(x−x̄)²] |
| Coefficient of Variation | CV = (σ/x̄) × 100 |
| GM | Antilog [1/n × Σ log x] |
| HM | n / Σ(1/x) |
| Karl Pearson's r | Σ(x−x̄)(y−ȳ) / √[Σ(x−x̄)²·Σ(y−ȳ)²] |
| Spearman's rs | 1 − [6ΣD² / n(n²−1)] |
| Gini Coefficient | G = Δ1 / 2X̄ |
| Nearest Neighbour Rn | 2D̄ × √(n/A) |
| Investment Multiplier | K = ΔY / ΔI |
